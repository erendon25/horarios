-- Allow a collaborator to claim an administrator-created staff record even
-- when the administrator did not know the collaborator's email in advance.
-- The email is still taken only from a confirmed Supabase Auth identity. If an
-- administrator did prefill an email, it must match that verified identity.
...
begin
  ...
  where store.id = v_staff.store_id
    and store.is_active;
  ...
  if nullif(lower(btrim(v_staff.email)), '') is not null
