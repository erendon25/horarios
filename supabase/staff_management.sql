...
-- Reingresos: mover una identidad Auth existente al nuevo perfil laboral sin
-- borrar el perfil cesado ni su historial operativo.
...
begin
  ...
  if nullif(lower(trim(p_email)), '') is null then raise exception 'El correo es obligatorio'; end if;
  ...
  select role into v_existing_role from public.user_profiles where id = p_user_id;
  ...
  update public.staff_profiles
  set user_id = p_user_id, email = lower(trim(p_email)), status = 'active',
  ...
  on conflict (id) do update set
    ...
    staff_profile_id = excluded.staff_profile_id,
    registration_pending = false, updated_at = now();
