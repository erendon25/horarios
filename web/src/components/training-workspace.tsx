...
import { buildTrainingEvidencePaths, isVerifiedTrainingCompletion, persistTrainingEvidencePair, resolveTrainingSignatureUrl, verifiedTrainingSkills } from "@/lib/training-evidence";
...
async function loadData(storeId: string) {
  ...
  const [people, evaluations] = await Promise.all([
    ...
    supabase.from("training_evaluations").select("id,staff_id,store_id,trainer_id,evaluation_date,area,station_code,station_name,score,responses,feedback,general_findings,status,current_step,collaborator_signature_path,trainer_signature_path,completion_verified_at,completion_version,is_edited,created_at,updated_at").eq("store_id", storeId).order("evaluation_date", { ascending: false }).order("id", { ascending: false }),
  ]);
...
async function getSignatureUrls(evaluation: Evaluation): Promise<SignatureUrls> {
  const create = async (path: string | null) => {
    return resolveTrainingSignatureUrl(path, async (storagePath) => {
      const { data, error } = await createClient().storage.from("training-signatures").createSignedUrl(storagePath, 900);
      if (error) throw error;
...
export function TrainingWorkspace({ role, userId, initialStoreId, ownStaffId }: { role: string; userId: string; initialStoreId: string | null; ownStaffId: string | null }) {
  ...
  const verifiedCompleted = completed.filter(isVerifiedTrainingCompletion);
  const historicalCompleted = completed.filter((row) => !isVerifiedTrainingCompletion(row));
  const drafts = data.evaluations.filter((row) => row.status === "draft");
  const certificationStaff = useMemo(() => data.staff.map((person) => ({
    ...person,
    skills: verifiedTrainingSkills(data.evaluations, person.id),
  })), [data.evaluations, data.staff]);
  const stats = useMemo(() => trainingStats(certificationStaff.map((person) => ({ id: person.id, name: name(person), skills: person.skills })), area), [certificationStaff, area]);
  ...
  const save = useMutation({ mutationFn: async (complete: boolean) => {
    ...
    if (complete) {
      ...
      const paths = buildTrainingEvidencePaths(selectedStore, id);
      const bucket = supabase.storage.from("training-signatures");
      await persistTrainingEvidencePair({
        paths,
        signatures: { collaborator: collaboratorBlob, trainer: trainerBlob },
        upload: (path, blob, options) => bucket.upload(path, blob, options),
        complete: async (evidencePaths) => {
          const { error } = await supabase.from("training_evaluations").update({
            status: "completed",
            collaborator_signature_path: evidencePaths.collaborator,
            trainer_signature_path: evidencePaths.trainer,
            score,
          }).eq("id", id).select("id").single();
          return { error };
        },
        remove: (evidencePaths) => bucket.remove(evidencePaths),
      });
    }
    return complete;
  ...
  return <>
    ...
    {!query.isPending && tab === "dashboard" && <Dashboard staff={certificationStaff} drafts={drafts} completed={verifiedCompleted} area={area} setArea={setArea} start={start} edit={edit}/>}
    ...
    {tab === "results" && <Results verified={verifiedCompleted} historical={historicalCompleted} staff={data.allStaff} view={setView}/>}
...
function Results({ verified, historical, staff, view }: { verified: Evaluation[]; historical: Evaluation[]; staff: Staff[]; view: (row: Evaluation) => void }) {
  return <div className="training-stack">
    <ResultTable title="Resultados verificados" subtitle={`${verified.length} evaluaciones válidas`} rows={verified} staff={staff} view={view} verified/>
    {historical.length > 0 && <ResultTable title="Histórico no verificado" subtitle={`${historical.length} registros conservados fuera de certificaciones y estadísticas`} rows={historical} staff={staff} view={view} verified={false}/>}
  </div>;
}

function ResultTable({ title, subtitle, rows, staff, view, verified }: { title: string; subtitle: string; rows: Evaluation[]; staff: Staff[]; view: (row: Evaluation) => void; verified: boolean }) {
  return <section className="data-card"><div className="data-card-heading"><div><strong>{title}</strong><span>{subtitle}</span></div></div><div className="table-scroll"><table className="training-results-table"><thead><tr><th>Fecha</th><th>Colaborador</th><th>Área / estación</th><th>Puntaje</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id}><td>{row.evaluation_date}</td><td>{name(staff.find((person) => person.id === row.staff_id))}</td><td><strong>{row.station_name}</strong><small>{row.area === "production" ? "Producción" : "Servicio"}</small></td><td><span className={`result-score ${verified && Number(row.score) >= 90 ? "certified" : ""}`}>{row.score ?? 0}%</span></td><td><span className={`status-pill ${verified && Number(row.score) >= 90 ? "active" : "pending"}`}>{verified ? (Number(row.score) >= 90 ? "Certificado" : "Reforzar") : "No verificada"}</span></td><td><div className="row-actions"><button className="icon-button" title="Ver" onClick={() => view(row)}><ClipboardCheck size={16}/></button></div></td></tr>)}{!rows.length && <tr><td colSpan={6} className="empty-row">Aún no hay resultados verificados.</td></tr>}</tbody></table></div></section>;
}
