...
const accessMessages: Record<string, string> = {
  ...
  registration: "Tu cuenta aún no tiene un vínculo válido. Selecciona tu tienda y ficha para enlazarla con tu correo confirmado.",
  invalid_link: "El enlace de recuperación venció, ya fue utilizado o no pertenece a este navegador. Solicita uno nuevo.",
};

export default async function LoginPage({ searchParams }: { searchParams: Promise<{ reason?: string; error?: string }> }) {
  const params = await searchParams;
  const messageKey = params.error ?? params.reason ?? "";
  return <main className="login-page">
    ...
    <section className="login-panel"><div className="auth-card"><div className="auth-card-heading"><span>ACCESO SEGURO</span><h2>Bienvenido de nuevo</h2><p>Ingresa con tu cuenta para continuar a tu espacio de trabajo.</p></div>{accessMessages[messageKey] && <p className="form-alert warning" role="status">{accessMessages[messageKey]}</p>}<LoginForm /><p className="migration-help">¿Es tu primer ingreso después de la migración? Usa <strong>¿Olvidaste tu contraseña?</strong></p></div><footer>Plataforma interna de operaciones</footer></section>
  </main>;
}
