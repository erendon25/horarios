...
const CANALES_FIJOS = ['SAL├ôN', 'DELIVERY', 'DRIVE THRU', 'SERV. FILA'];
const DIAS_SEMANA = ['dom', 'lun', 'mar', 'mi├®', 'jue', 'vie', 's├íb'];
...
export default function SalesAnalysis() {
    ...
    // LECTURA DE M├ÜLTIPLES TABLAS EN INFOREST
    ...
    const processSalesRows = async (data) => {
        ...
        // B├║squeda inteligente evitando "Subtotales" o "Dsctos"
        ...
        const estadoKey = allKeys.find(k => {
            ...
            return c === 'estado' || c === 'status' || c === 'estado pedido' ||
                   c === 'estatus' || c === 'condicion' || c === 'condici├│n' ||
                   c === 'tipo' || c === 'situacion' || c === 'situaci├│n';
        }) || null;
        ...
        data.forEach((fila, index) => {
            ...
            else if (canalRawStr.includes('FILA') || canalRawStr.includes('MODULO') || canalRawStr.includes('M├ôDULO')) canal = 'SERV. FILA';
            else if (canalRawStr.includes('LOCAL') || canalRawStr.includes('SALON') || canalRawStr.includes('SAL├ôN')) canal = 'SAL├ôN';
            else if (canalRawStr !== '') canal = 'SAL├ôN';
            ...
            // Deduplicar por n├║mero de comprobante:
            // Si ya procesamos este pedido en este mismo d├¡a, no sumarlo dos veces.
        ...
        console.log(`[SalesAnalysis] DIAGN├ôSTICO DE PROCESAMIENTO:`);
        console.log(`  - Total filas en archivo: ${data.length}`);
        console.log(`  - Filas v├ílidas procesadas: ${debugRows}`);
        console.log(`  - Filas ignoradas/saltadas: ${debugSkipped}`);
        console.log(`  - SUMA TOTAL CALCULADA: S/ ${debugSum.toFixed(2)}`);
        console.log(`  - D├¡as operativos: ${Object.keys(dailyAggregations).length}`);
        ...
        alert(`┬íDatos guardados con ├®xito! Se reescribieron y consolidaron ${Object.keys(dailyAggregations).length} d├¡as.`);
    };
    ...
    const AnalysisSection = ({ title, compareData, compareLabel, colorCompare, colorCurrent, viewMode }) => {
        ...
        const calcVar = (act, ant) => {
            ...
            const dif = act - ant;
            const isPos = dif >= 0;
            return { pct, dif, color: isPos ? 'text-green-600' : 'text-red-600', icon: isPos ? 'Ôû▓' : 'Ôû╝' };
        };
                ...
                <div className="flex flex-col lg:flex-row gap-6">
                    ...
                    <div className="flex-1 grid grid-cols-1 xl:grid-cols-2 gap-6">
                        ...
                        <div className="bg-white border border-gray-200 shadow-sm flex flex-col relative pt-4 xl:col-span-2 mt-4">
                            <h3 className="text-center font-bold text-red-700 italic mb-2">{isTxs ? 'Transacciones por d├¡a' : 'Ventas por d├¡a'}</h3>
                            <div className="h-64 px-2">
                                <ResponsiveContainer width="100%" height="100%">
                                    ...
                                    return (
                                        <>
                                            ...
                                            <span className={`text-lg font-bold px-2 py-1 rounded bg-gray-50 ${colorClass}`}>
                                                {isPos ? 'Ôû▓' : 'Ôû╝'} {Math.abs(pct).toFixed(1)}%
                                            </span>
                                        </>
                                    );
                                })()}
