...
const TrainingDashboard = ({ onStartEvaluation, activeArea, onAreaChange, onSelectCollaborator, onShowStats, drafts = [], loadingDrafts = false, onSelectDraft }) => {
    ...
    return (
        <div className="flex flex-col min-h-[100dvh] bg-slate-50/50 pb-8 sm:pb-20">
            {/* Header */}
            <div className="bg-white p-4 sm:p-6 rounded-b-3xl sm:rounded-b-[40px] shadow-sm border-b border-gray-100">
                <div className="flex justify-between items-start mb-6">
                    <div>
                        <h1 className="text-2xl sm:text-3xl font-black text-gray-900 mb-1 tracking-tight uppercase">
                            {activeArea === 'service' ? 'Servicio' : 'Producción'}
                        </h1>
                        <p className="text-[10px] sm:text-[11px] text-gray-400 font-black uppercase tracking-[0.12em] sm:tracking-[0.2em] leading-relaxed">
                            {activeArea === 'service' ? 'Certificación de servicio al cliente' : 'Certificación de procesos de cocina'}
                        </p>
                    </div>
                </div>

                <div className="bg-gray-100 p-1.5 rounded-2xl flex mb-6 sm:mb-8 shadow-inner">
                    <button
                ...
                <div className="flex gap-2 sm:gap-4">
                    <button
                        ...
                        className="flex-1 min-w-0 bg-orange-500 hover:bg-orange-600 text-white py-4 px-3 sm:px-6 rounded-2xl sm:rounded-[24px] font-black uppercase tracking-[0.08em] sm:tracking-[0.15em] text-[9px] sm:text-[10px] flex items-center justify-center gap-2 sm:gap-3 transition-all shadow-xl shadow-orange-500/30 active:scale-95 border-b-4 border-orange-700"
                    >
                    ...
                    <button
                        onClick={() => onShowStats?.()}
                        className="shrink-0 p-4 bg-white text-orange-500 rounded-2xl border border-orange-100 shadow-sm transition-all hover:bg-orange-50 hover:scale-105 active:scale-95"
            ...
            {/* Drafts Section */}
            {drafts.length > 0 && (
                <div className="px-4 sm:px-6 mt-6 sm:mt-8 animate-in fade-in slide-in-from-top duration-700">
                    ...
                    <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide -mx-2 px-2">
                        {drafts.map((draft) => (
                            <button
                                ...
                                className="shrink-0 w-[min(16rem,calc(100vw-3rem))] bg-white p-4 sm:p-5 rounded-2xl sm:rounded-[32px] border border-orange-100 shadow-xl shadow-orange-500/5 hover:shadow-orange-500/15 transition-all group text-left relative overflow-hidden active:scale-95"
                            >
            ...
            {/* Search and Stats */}
            <div className="px-4 sm:px-6 mt-6 sm:mt-8 flex-1 flex flex-col min-h-0">
                <div className="relative mb-5 sm:mb-8">
                    <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-300" size={20} />
                    <input
                        type="text"
                        placeholder="BUSCAR COLABORADOR..."
                        className="w-full pl-14 pr-4 sm:pr-6 py-4 sm:py-5 bg-white border border-gray-100 rounded-2xl sm:rounded-[32px] focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 outline-none transition-all shadow-xl shadow-gray-200/50 font-bold text-sm tracking-tight"
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                    />
                </div>

                <div className="flex gap-2 mb-5 sm:mb-8 overflow-x-auto pb-2 sm:pb-4 scrollbar-hide">
                    {['all', 'pending', 'training', 'certified'].map((tab) => (
                        <button
                            key={tab}
                            onClick={() => setFilter(tab)}
                            className={`px-4 sm:px-8 py-3 rounded-full whitespace-nowrap text-[9px] sm:text-[10px] font-black uppercase tracking-[0.12em] sm:tracking-[0.2em] transition-all border ${filter === tab
                ...
                {/* Collaborators List */}
                <div className="flex-1 pb-16 sm:pb-32 custom-scrollbar">
                    {loading ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
                            {filteredCollaborators.map((collab, idx) => (
                                <div
                                    ...
                                    className="bg-white p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-[32px] lg:rounded-[40px] border border-gray-100 shadow-xl sm:shadow-2xl shadow-gray-200/60 hover:shadow-orange-500/20 hover:border-orange-100 transition-all duration-500 cursor-pointer group flex flex-col justify-between h-full relative overflow-hidden animate-in fade-in slide-in-from-bottom"
                                >
                                    <div className="flex items-center gap-4 sm:gap-6 mb-6 sm:mb-8">
                                        <div className="relative shrink-0">
                                            <div className="w-14 h-14 sm:w-20 sm:h-20 bg-gradient-to-br from-orange-50 to-white rounded-2xl sm:rounded-[24px] flex items-center justify-center text-orange-600 font-black text-2xl sm:text-3xl border border-orange-100 shadow-inner group-hover:scale-110 transition-transform duration-500">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <h3 className="font-black text-slate-900 text-lg sm:text-2xl tracking-tight sm:tracking-tighter mb-1 break-words leading-tight">{collab.name}</h3>
                                    ...
                                    <div className="pt-5 sm:pt-6 border-t border-gray-50 flex justify-between items-center bg-gradient-to-b from-transparent to-gray-50/30 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8 -mb-4 sm:-mb-6 lg:-mb-8 mt-2 pb-4 sm:pb-6 lg:pb-8">
                                        ...
                                        <button
                                            ...
                                            className="w-11 h-11 sm:w-14 sm:h-14 shrink-0 bg-orange-500 rounded-xl sm:rounded-2xl flex items-center justify-center text-white hover:bg-orange-600 transition-all duration-300 shadow-lg shadow-orange-500/30 active:scale-95 z-20 group/btn"
